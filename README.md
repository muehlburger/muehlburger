### Hi there 👋

I'm Herbert Mühlburger — founder of [AI-Trust ZT GmbH](https://www.it-zt.at), a chartered engineering consultant firm in Austria specializing in AI consulting and software engineering. I'm also a PhD candidate at TU Graz, researching ML-based fault detection in cyber-physical systems.

---

#### 👨‍💻 What I do
- 🎓 PhD research at TU Graz — applying deep learning to fault detection across industrial and urban systems
- 🏢 Running a consulting practice focused on AI strategy, software architecture, and digital transformation
- 🔧 Building open-source tools to simplify complex workflows
- 🧠 Bridging traditional software engineering with modern AI methods

#### 📄 Selected publications
- Mühlburger & Wotawa (2026). *RAMP: A Region-as-Sensor Abstraction for Deriving Planner-Oriented Mobility Indicators from Municipal Webcam Feeds.* (under review)
- Mühlburger & Wotawa (2024). *Making Systems Fail-Aware.* IFAC-PapersOnLine 58(4).
- Mühlburger & Wotawa (2022). *A Passive Testing Approach using a Semi-Supervised Intrusion Detection Model for SCADA Network Traffic.* IEEE AITest.

#### 🤝 Let's connect
- 🛠️ Always open to collaboration on open-source projects and meaningful AI applications
- 🗣️ Ask me about: AI/ML · Software Architecture · Cloud · Kubernetes · Go · Microservices · Startups
- 📨 Reach me via [Twitter/X](https://twitter.com/hmuehlburger) or [LinkedIn](https://www.linkedin.com/in/herbertmuehlburger/)

#### ⚡ Fun fact
I grew up in Osttirol and can run a mountain farm at 1,200m altitude — including a vineyard 🌱🍇
